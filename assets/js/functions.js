...
function leaveCancel(id) {
	$.ajax({
		url:"http://172.104.162.155/leave-cancel/" + id,
		method:"POST",
		success: function(data) {
			toastr.success('Leave has been cancelled', 'Success', {timeOut: 8000})
			upcomingLeave();
			window.setTimeout(function(){window.location.href = "http://172.104.162.155/" }, 5000);
		}
	});
}
...
function upcomingLeave() {
	$.ajax({
		url:"http://172.104.162.155/upcoming-leave",
		method:"POST",
		success: function(data) {
...
function msgHide(id) {
	$.ajax({
		url:"http://172.104.162.155/comment-hide/" + id,
		method:"POST",
		success: function(data) {
			_getFeedBack();
