	...
	public function form() {
		...
		if($this->form_validation->run()) {
			...
			if($this->comment->insertMessage($data)) {
				...
				save_action(array('module' => Constant::M_DASHBOARD, 'action' => Constant::A_ADD, 'object_id' => $this->db->insert_id()));
			}
	...
	public function edit() {
		...
		if($this->form_validation->run()) {
			...
			if($this->user->updateMydata($data,$id)) {
				...
				save_action(array('module' => Constant::M_DASHBOARD, 'action' => Constant::A_UPDATE, 'object_id' => $id));
	...
	*/
	...
	public function getEmployee() {
		...
		foreach ($data as $val) {
			...
			if($val->vacationleave == Constant::D_LEAVE) {
				$v_l = 0;
				$vl = 0;
			} else {
			...
			if($val->sickleave == Constant::D_LEAVE) {
				$s_l = 0;
				$sl = 0;
			} else {
			...
			if($this->session->userdata('uname') == $val->username) {
				if($v_l < 3){
					$modal = 'class="leave-wrapper-inactive warning" data-toggle="modal" data-target="#editProfile"';
				} else {
					$modal = 'class="leave-wrapper-inactive" data-toggle="modal" data-target="#editProfile"';
				}
			} else {
				if($v_l < 3){
					$modal = 'class="leave-wrapper warning" data-id="'.$val->id.'"';
				} else {
				...
				<div class="col-sm">
					<div '.$modal.'>
						...
						<div class="lv-info">
							...
							<div class="lv-details vl-'.$val->employee_id.'"><span>'.$v_l.'</span>/'.$vl.'</div>
						</div>
						<div class="lv-info">
							<div class="lv-label">SL</div>
							<div class="lv-details sl-'.$val->employee_id.'"><span>'.$s_l.'</span>/'.$sl.'</div>
						</div>
						<div class="lv-info">
